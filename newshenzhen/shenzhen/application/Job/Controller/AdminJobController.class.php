...
class AdminJobController extends AdminbaseController {
	...
	function set_post(){
		if (IS_POST) {
			...
			$post_id = $_POST['id'];
			unset($_POST['id']);
			$_POST['info']=json_encode($_POST['info']);
			$_POST['validity_data'] = strtotime($_POST['validity_data']);
			$_POST['respon']=htmlspecialchars_decode($_POST['respon']);
			$_POST['resquest']=htmlspecialchars_decode($_POST['resquest']);
			$_POST['etc']=htmlspecialchars_decode($_POST['etc']);
			
			$is_real = $this->job_post_model->where("job_id=$job_id")->find();
			if(empty($is_real)){
				$result=$this->job_post_model->add($_POST);
				if(!empty($_POST['ids'])){
					$this->set_relationship($result,$_POST['ids']);
				}
			}else{
				$result=$this->job_post_model->where("id=$post_id")->save($_POST);
				if(!empty($_POST['ids'])){
