...
class RelationshipController extends AdminbaseController {
	...
	function index(){
		...
		foreach ($result as $r) {
			...
			$url = U("Relationship/edit", array("parent" => $r['term_id']));
			$r['str_manage'] = '<a href="javascript:open_iframe_layer('."'$url'".','."'关联问答分类选择'".');">配置</a>';
			$r['taxonomys'] = $this->taxonomys[$r['taxonomy']];
			$r['id']=$r['term_id'];
			$r['parentid']=$r['parent'];
			$r['link_name']='';
			$question = $this->link_model->where("term_id=".$r['term_id'].' and type=1')->select();
	...
	function edit(){
		...
		foreach ($result as $r) {
			...
			$question = $this->link_model->where('type=1 and question_id='.$r['term_id'].' and term_id='.$tid)->find();
			if($question){
			...
			if(sp_is_question_child_terms($r['term_id'])){
				$r['type'] = 'disabled';
			}else{
				$r['type'] = '';
			}
		...
		/*$tid = intval(I("get.parent"));
		$term = $this->terms_model->where("term_id=".$tid)->find();
		$qaterm = $this->qterms_model->where('parent=0')->select();
	...
	function edit_post(){
		...
		if (IS_POST) {
			$del = $this->link_model->where('term_id='.$parent.' and type=1')->delete();
			foreach($aid as $k=>$v){
				$rel['term_id'] = $parent;
				$rel['question_id'] = $k;
				$rel['link_name'] = $v;
