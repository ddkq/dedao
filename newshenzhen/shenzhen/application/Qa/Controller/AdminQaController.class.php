...
class AdminQaController extends AdminbaseController {
	...
	function _initialize() {
		parent::_initialize();
		$this->qa_model = D("Protal/Qa");
		//var_dump($this->qa_model);exit;
		$this->terms_model = D("Qa/Terms");
		$this->term_relationships_model = D("Qa/TermRelationships");
	}
	function index(){
		$this->_lists();
		$this->_getTree();
	...
	function add_post(){
		if (IS_POST) {
			...
			$result=$this->qa_model->add($article);
			//$this->term_relationships_model->where("object_id=".$result)->delete();
			//echo $this->qa_model->_sql();
			if ($result) {
				//
				foreach ($_POST['term'] as $mterm_id){
					$this->term_relationships_model->add(array("term_id"=>intval($mterm_id),"object_id"=>$result,"isqa"=>1));
				}
...
public function edit(){
		...
		$term_relationship = M('TermRelationships')->where(array("object_id"=>$id,"status"=>1,"isqa"=>1))->getField("term_id",true);
		$this->_getTermTree($term_relationship);
		$terms=$this->terms_model->select();
		$post=$this->qa_model->where("id=$id")->find();
	...
	public function edit_post(){
		if (IS_POST) {
			...
			$this->term_relationships_model->where(array("object_id"=>$post_id,"isqa"=>1,"term_id"=>array("not in",implode(",", $_POST['term']))))->delete();
			//echo $this->term_relationships_model->_sql();
			foreach ($_POST['term'] as $mterm_id){
				$find_term_relationship=$this->term_relationships_model->where(array("object_id"=>$post_id,"term_id"=>$mterm_id,"isqa"=>1))->count();
				if(empty($find_term_relationship)){
					$this->term_relationships_model->add(array("term_id"=>intval($mterm_id),"object_id"=>$post_id,"isqa"=>1));
				}else{
					$this->term_relationships_model->where(array("object_id"=>$post_id,"term_id"=>$mterm_id,"isqa"=>1))->save(array("status"=>1,"isqa"=>1));
				}
			}
			...
			$article['post_phone']=htmlspecialchars_decode($article['post_phone']);
			//var_dump($article);exit;
			$result=$this->qa_model->save($article);
			if ($result!==false) {
				$this->success("保存成功！");
			} else {
				$this->error("保存失败！");
	...
	private  function _lists($status=1){
		...
		->alias("a")
		->join(C ( 'DB_PREFIX' )."qa b ON a.object_id = b.id")
		->where($where)
		->count();
	...
	private function _getTermTree($term=array()){
		$result = $this->terms_model->where('parent=62 or parent=3 or parent=4 or parent=5 or parent=12 or parent=13 or parent=16  or parent=17  or parent=28  or parent=96')->order(array("listorder"=>"asc"))->select();
		...
		$taxonomys = $tree->get_tree(62, $str);
	...
	function delete(){
		if(isset($_GET['tid'])){
			...
			$data['status']=0;
			if ($this->term_relationships_model->where("tid=$tid and isqa=1")->save($data)) {
				$this->success("删除成功！");
			} else {
				$this->error("删除失败！");
			}
		...
		if(isset($_POST['ids'])){
			$tids=join(",",$_POST['ids']);
			$data['status']=0;
			if ($this->term_relationships_model->where("tid in ($tids) and isqa=1")->save($data)) {
				$this->success("删除成功！");
			} else {
				$this->error("删除失败！");
			}
	...
	function check(){
		if(isset($_POST['ids']) && $_GET["check"]){
			...
			$tids=join(",",$_POST['ids']);
			$objectids=$this->term_relationships_model->field("object_id")->where("tid in ($tids) and isqa=1")->select();
			$ids=array();
			foreach ($objectids as $id){
				$ids[]=$id["object_id"];
			}
			$ids=join(",", $ids);
			if ( $this->qa_model->where("id in ($ids)")->save($data)!==false) {
				$this->success("审核成功！");
			} else {
				$this->error("审核失败！");
			}
		...
		if(isset($_POST['ids']) && $_GET["uncheck"]){
			...
			$ids=join(",", $ids);
			if ( $this->qa_model->where("id in ($ids)")->save($data)) {
				$this->success("取消审核成功！");
			} else {
				$this->error("取消审核失败！");
			}
	...
	function move(){
		if(IS_POST){
			if(isset($_GET['ids']) && isset($_POST['term_id'])){
				$tids=$_GET['ids'];
				if ( $this->term_relationships_model->where("tid in ($tids) and isqa=1")->save($_POST) !== false) {
					$this->success("移动成功！");
				} else {
					$this->error("移动失败！");
				}
	...
	function clean(){
		if(isset($_POST['ids'])){
			...
			$status=$this->term_relationships_model->where("tid in ($tids) and isqa=1")->delete();
			if($status!==false){
				foreach ($_POST['ids'] as $post_id){
					$post_id=intval($post_id);
					$count=$this->term_relationships_model->where(array("object_id"=>$post_id))->count();
					if(empty($count)){
						$status=$this->qa_model->where(array("id"=>$post_id))->delete();
					}
				}
			...
			if(isset($_GET['id'])){
				$id = intval(I("get.id"));
				$tid = intval(I("get.tid"));
				$status=$this->term_relationships_model->where("tid = $tid and isqa=1")->delete();
				if($status!==false){
					$count=$this->term_relationships_model->where(array("object_id"=>$id,"isqa"=>1))->count();
					if(empty($count)){
						$status=$this->qa_model->where("id=$id")->delete();
					}
	...
	function restore(){
		if(isset($_GET['id'])){
			...
			if ($this->term_relationships_model->where("isqa=1")->save($data)) {
				$this->success("还原成功！");
			} else {
				$this->error("还原失败！");
			}
