	...
	<div class="wrap js-check-wrap">
		...
		<form class="js-ajax-form" action="" method="post">
			<div class="table-actions">
				...
				<button class="btn btn-primary btn-small js-ajax-submit" type="submit" data-action="<?php echo U('AdminPost/collection');?>" data-subcheck="true">导入</button>
			</div>
			<table class="table table-hover table-bordered table-list">
				...
				<?php if(is_array($posts)): foreach($posts as $key=>$vo): ?><tr>
					...
					<td>
						<?php if($vo["link_type"] == 0){ $url = "/article/".$vo['tid'].".html"; }else{ if(empty($vo["post_url"])){ $url = $vo["post_murl"]; }else{ $url = $vo["post_url"]; } } ?>
						<a href=<?php echo ($url); ?> target="_blank"> <span><?php echo ($vo["post_title"]); ?></span></a>
					</td>
					...
					<td><?php echo ($vo["post_modified"]); ?></td>
