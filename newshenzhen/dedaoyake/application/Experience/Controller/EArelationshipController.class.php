...
class EArelationshipController extends AdminbaseController {
	...
	function index(){
		...
		foreach ($result as $r) {
			...
			$r['parentid']=$r['parent'];
			$r['link_name']='';
			$question = $this->link_model->where("term_id=".$r['term_id'].' and type=3')->select();
	...
	function edit(){
		...
		foreach ($result as $r) {
			...
			$question = $this->link_model->where('type=3 and ex_id='.$r['term_id'].' and term_id='.$tid)->find();
			if($question){
	...
	function edit_post(){
		...
		if (IS_POST) {
			$del = $this->link_model->where('term_id='.$parent.' and type=3')->delete();
			foreach($aid as $k=>$v){
				$rel['term_id'] = $parent;
				$rel['ex_id'] = $k;
				$rel['link_name'] = $v;
