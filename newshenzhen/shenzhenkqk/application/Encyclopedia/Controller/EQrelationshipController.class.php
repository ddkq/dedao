...
class EQrelationshipController extends AdminbaseController {
	...
	function index(){
		...
		foreach ($result as $r) {
			...
			$r['parentid']=$r['parent'];
			$r['link_name']='';
			$question = $this->link_model->where("question_id=".$r['term_id'].' and type=5')->select();
	...
	function edit(){
		...
		foreach ($result as $r) {
			...
			$question = $this->link_model->where('type=5 and en_id='.$r['term_id'].' and question_id='.$tid)->find();
			if($question){
	...
	function edit_post(){
		...
		if (IS_POST) {
			$del = $this->link_model->where('question_id='.$parent.' and type=5')->delete();
			foreach($aid as $k=>$v){
