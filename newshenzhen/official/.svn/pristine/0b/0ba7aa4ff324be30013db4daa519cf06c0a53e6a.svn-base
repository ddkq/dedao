...
class AdminExTermController extends AdminbaseController {
	...
	function index(){
		...
		$tree->nbsp = '&nbsp;&nbsp;&nbsp;';
		foreach ($result as $r) {
			$r['str_manage'] = '<a href="' . U("AdminExTerm/add", array("parent" => $r['term_id'])) . '">'.L('ADD_SUB_CATEGORY').'</a> | <a href="' . U("AdminExTerm/edit", array("id" => $r['term_id'])) . '">'.L('EDIT').'</a> | <a class="js-ajax-delete" href="' . U("AdminExTerm/delete", array("id" => $r['term_id'])) . '">'.L('DELETE').'</a> ';
			$url="/experience/list/".$r['term_id'].".html";
			$r['url'] = $url;
		...
		if (IS_POST) {
			if ($this->exterms_model->create()) {
				if ($this->exterms_model->add()!==false) {
				    F('all_terms',null);
					$this->success("添加成功！",U("AdminExTerm/index"));
				} else {
					$this->error("添加失败！");
				}
			} else {
