...
class AdminTermController extends AdminbaseController {
	...
	protected $term_relationship_model;
	protected $taxonomys=array("article"=>"文章","picture"=>"图片");
	
	function _initialize() {
		parent::_initialize();
		$this->terms_model = D("Portal/Terms");
		$this->term_relationship_model = D("Portal/TermRelationships");
	...
	function index(){
		...
		foreach ($result as $r) {
			...
			$url="/list/".$r['term_id'].".html";
			$r['url'] = $url;
			$r['taxonomys'] = $this->taxonomys[$r['taxonomy']];
			$r['id']=$r['term_id'];
			$r['count'] = $this->term_relationship_model->where(['term_id'=>$r['term_id'],'status'=>1])->count();
			$r['parentid']=$r['parent'];
			$array[] = $r;
		}
