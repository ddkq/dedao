...
use app\encyclopedia\model\CategoryModel;
use app\encyclopedia\validate\EnCategoryValidate;
use think\Db;
...
class AdminCategory extends AdminBase{
	...
	public function addPost(){
		if($this->request->isPost()){
			...
			$result = $this->validate($data, 'EnCategoryValidate');
			if($result !== true){
				$this->info(0,$result);
			}
	...
	public function delete(){
		...
		$result = Db::connect('db_config'.parent::$db_id)->name('en_category')->where('id',$id)->setField('delete_time',time());
		if($result){
			$this->info(1,'删除成功');
		}else{
			$this->info(0,'删除失败');
