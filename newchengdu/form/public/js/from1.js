$(function() {
	...
	$('body').on('click', '.add-line a', function() {
		var addHtml = `<tr class="second">
						<td class="name"><input type="text" placeholder="选项"></td>
						<td class="type">选项</td>
						<td class="must"></td>
						<td class="ctr">
		...
		$(this).parents('tr').before(addHtml);
	...
	$('body').on('click', '.add-table a', function() {
		var html2 = '';
		for(var i = 0; i < attr.length; i++) {
			if(i == 7) {
		...
		var addHtml = `<table>
							...
							<tr class="first">
								<td class="name"><input type="text" name="" value="标题"></td>
								<td class="type">
									<select name="" id="select">${html2}</select>
								</td>
								<td class="must">
									<div class="cheakbox">
							...
							<tr class="second">
								<td class="name"><input type="text" name="" value="选项"></td>
								<td class="type">选项</td>
								<td class="must"></td>
								<td class="ctr">
									<span class="btn btn-default" title="上移">
							...
							<tr class="second">
								<td><div class="add-line"><a href="javascript:;"><i>+</i>增加一项</a></div></td>
								<td></td>
								<td></td>
								<td></td>
							</tr>
						</table>`;
		$('.table-box').append(addHtml);
	...
	function setInputName(){
		$.each($(".table-box table"),function(i,n){
			$(this).find('tr.first').find('.name').find('input').attr('name','content['+i+'][name]');
			$(this).find('tr.first').find('.type').find('select').attr('name','content['+i+'][type]');
			$.each($(this).find('tr.second'), function(j) {
				$(this).find('.name').find('input').attr('name','content['+i+'][children]['+j+'][name]');
				$(this).find('.must').find('input').attr('name','content['+i+'][children]['+j+'][is_must]');
			});
		});
	}
	setInputName();
