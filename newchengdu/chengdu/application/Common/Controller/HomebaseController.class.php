...
class HomebaseController extends AppframeController {
	...
	function _initialize() {
		...
		if(sp_is_mobile()){
			$nav = sp_get_menu_tree(5);
			$this->assign('nav',$nav);
		}else{
			$article_nav = sp_get_article_nav();
			$this->assign("article_nav",$article_nav);
			$ad_slides = sp_getslide(swt_slide);
			$this->assign("ad_slides",$ad_slides);
		}
	...
	public function parseTemplate($template='') {
		...
		C("TMPL_PARSE_STRING.__TMPL__Zt",__ROOT__."/".$current_tmpl_path."zt");
		C("TMPL_PARSE_STRING.__TMPL__ZT",__ROOT__."/".$current_tmpl_path."zt");
