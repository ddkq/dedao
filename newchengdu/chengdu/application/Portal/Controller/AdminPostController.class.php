...
class AdminPostController extends AdminbaseController {
	...
	function add(){
		...
		$istop=$this->ac_model->where('cat_value = 2')->select();
		$recommended=$this->ac_model->where('cat_value = 3')->select();
		$select=$this->ac_model->where('cat_value = 1')->select();
	...
	function add_post(){
		if (IS_POST) {
			...
			if(!empty($_POST['headline-photos_alt']) && !empty($_POST['headline-photos_url'])){
				foreach ($_POST['headline-photos_url'] as $key=>$url){
					$photourl=sp_asset_relative_url($url);
					$_POST['smeta']['headline-photo'][]=array("url"=>$photourl,"alt"=>$_POST['headline-photos_alt'][$key]);
				}
			}
			...
			$article=I("post.post");
			if(empty($article['post_subtitle'])){
				$article['post_subtitle'] = $article['post_title'];
			}
			if(empty($article['post_mtitle'])){
				$article['post_mtitle'] = $article['post_title'];
			}
			$article['smeta']=json_encode($_POST['smeta']);
			$article['post_content']=htmlspecialchars_decode($article['post_content']);
			$article['post_phone']=htmlspecialchars_decode($article['post_phone']);
			$article['post_extend']=json_encode($_POST['extend']);
	...
	function collection1(){
		$collection_model = M('Content');
		$collection = $collection_model->select();
		//var_dump($collection);
		...
		foreach($collection as $vo){
			...
			$content = json_decode($vo['data'],true);
			if(empty($content)){
				continue;
			};
			//var_dump($content);exit;
			$article['post_title'] = $content['title'];
			$article['post_subtitle'] = $content['title'];
			$article['post_mtitle'] = $content['title'];
			$article['post_excerpt'] = $content['author'];
			$article['select'] = $content['select'];
			//$article['post_content'] = stripHtmlTags(array('script'),$content['content']);
			$preg = "/<script[\s\S]*?<\/script>/i";
			$str = preg_replace($preg,"",$content['content']);
			$str1 = str_replace('");</script>',"",$str);
			
			$pattern = "/<P align=right>(.*?)<\/p>/";  
			$str2 = preg_replace($pattern,"",$str1); 
			$str3 = str_replace('广州现代肿瘤医院(www.39.net)专稿，未经书面授权请勿转载。',"",$str2);
			$article['post_content'] = str_replace('（以上内容仅授权广州现代肿瘤医院独家使用，未经版权方授权请勿转载。）',"",$str3);
			$article['post_phone'] = $article['post_content'];
			
			//var_dump($article['post_content'] );exit;
			$result=$this->posts_model->add($article);
			if ($result){
				$this->term_relationships_model->add(array("term_id"=>85,"object_id"=>$result));
			}
		//exit;	
		}
	...
	function collection(){
		...
		foreach($collection as $vo){
			...
			//$str1 = str_replace('");</script>',"",$str);
			//$preg1 = "/<script([^>]*)>/";
			//$str2 = preg_replace($preg1,"",$str1);
			
			//$str3 = str_replace('广州现代肿瘤医院(www.39.net)专稿，未经书面授权请勿转载。',"",$str2);
			
			$preg = '/<div([^>]*)>(.*)<\/div>/i';
			$str = stripHtmlTags(array("div","dl"),$content['content'],true);
			
			$str1 = str_replace('</div>',"",$str);
			
			$preg1 = "/<script[\s\S]*?<\/script>/i";
			$str2 = preg_replace($preg1,"",$str1);
			
			$str3 = str_replace('");</script>',"",$str2);
			
			$preg2 = "/<div([^>]*)>/";
			$str4 = preg_replace($preg2,"",$str3);
			
			$preg3 = "/<script([^>]*)>/";
			$str5 = preg_replace($preg3,"",$str4);
			
			$article['post_content'] = $str5;
			$article['post_phone'] = $article['post_content'];
			
			//var_dump($article['post_content'] );exit;
			$result=$this->posts_model->add($article);
			if ($result){
				$this->term_relationships_model->add(array("term_id"=>107,"object_id"=>$result));
			}
		//exit;	
		}
