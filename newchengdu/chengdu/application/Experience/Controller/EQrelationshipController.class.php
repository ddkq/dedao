...
class EQrelationshipController extends AdminbaseController {

	protected $qterms_model;
	protected $eterms_model;
	...
	function _initialize() {
		...
		$this->eterms_model = D("Experience/Exterms");
	...
	function index(){
		...
		foreach ($result as $r) {
			...
			$url = U("EQrelationship/edit", array("parent" => $r['term_id']));
			$r['str_manage'] = '<a href="javascript:open_iframe_layer('."'$url'".','."'关联百科分类选择'".');">配置</a>';
			$r['taxonomys'] = $this->taxonomys[$r['taxonomy']];
			$r['id']=$r['term_id'];
			$r['parentid']=$r['parent'];
			$r['link_name']='';
			$question = $this->link_model->where("question_id=".$r['term_id'].' and type=2')->select();
	...
	function edit(){
		...
		$result = $this->eterms_model->order(array("listorder"=>"asc"))->select();
		...
		foreach ($result as $r) {
			...
			$question = $this->link_model->where('type=2 and ex_id='.$r['term_id'].' and question_id='.$tid)->find();
			if($question){
			...
			if(sp_is_ex_child_terms($r['term_id'])){
				$r['type'] = 'disabled';
			}else{
				$r['type'] = '';
			}
		...
		/*$tid = intval(I("get.parent"));
		$term = $this->qterms_model->where("term_id=".$tid)->find();
		$qaterm = $this->eterms_model->where('parent=0')->select();
	...
	function edit_post(){
		...
		if (IS_POST) {
			$del = $this->link_model->where('question_id='.$parent.' and type=2')->delete();
			foreach($aid as $k=>$v){
