$(function() {
	...
	$('.next').on('click', function() {
		$('.banner-tab .doctor-tab-active').next().addClass('doctor-tab-active').siblings('li').removeClass('doctor-tab-active');
		var inx = $('.banner-tab .doctor-tab-active').index();
		mySwiper.slideTo(inx, 500, false);
		inx == 1 ? ($('.doctor-bcg-arrow').animate({'left': '65%'})) | (remark = 0) : $('.doctor-bcg-arrow').animate({'left': '20%'});
	});
	$('.prev').on('click', function() {
		$('.banner-tab .doctor-tab-active').prev().addClass('doctor-tab-active').siblings('li').removeClass('doctor-tab-active');
		var inx = $('.banner-tab .doctor-tab-active').index();
		mySwiper.slideTo(inx, 500, false);
		inx == 0 ? $('.doctor-bcg-arrow').animate({'left': '20%'}) : $('.doctor-bcg-arrow').animate({'left': '65%'});
	});
