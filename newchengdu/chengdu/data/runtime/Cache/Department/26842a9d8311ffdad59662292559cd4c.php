	...
	<div class="brand_left_bd">
		<ul>
			...
			<div class="brand_right">
				...
				<div class="brand_zj_intro">
					德道口腔拥有严格的医师准入制度，每一位进入德道的医师都必须经过层层考核。目前，在口腔种植、口腔正畸、口腔修复、牙齿美白、牙周治疗、儿童口腔等各领域均有资深专家及领军人才储备，在学科带头人的领导下，各学科组潜心钻研，夯实学科理论基础，提高临床实操技能，致力于为广大消费者提供专业、权威的口腔诊疗服务。
				</div>
				<div class="brand_zj">
					<div class="con">
						<ul class="clearfix">
							<?php if(is_array($all_doctor)): $i = 0; $__LIST__ = array_slice($all_doctor,0,20,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="clearfix w-48 d-inline-block p-3 border mr-2 position-relative">
									<div class="zj_pic"><img src="<?php echo ($vo["main_img"]); ?>"></div>
									<div class="zj_des">
										<div class="zj_title"><?php echo ($vo["name"]); ?> <strong><?php echo ($vo["job"]); ?></strong></div>
										<div class="zj_ry">
											<?php if(is_array($vo['experience'])): $i = 0; $__LIST__ = $vo['experience'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo1): $mod = ($i % 2 );++$i;?><p><?php echo ($vo1); ?></p><?php endforeach; endif; else: echo "" ;endif; ?>
										</div>
										<div class="zj_xm">擅长项目：<?php echo ($vo["item"]); ?></div>
									</div>
									<div class="zj_hover">
										<div class="text">
											<?php if(is_array($vo['experience'])): $i = 0; $__LIST__ = array_slice($vo['experience'],0,3,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo1): $mod = ($i % 2 );++$i;?><p style="height: 21px;"><?php echo ($vo1); ?></p><?php endforeach; endif; else: echo "" ;endif; ?>
										</div>
										<i class="fa fa-chevron-circle-down"></i>
										<div class="swt">
											<a href="<?php echo get_doctor_url($vo[id],$vo[link_type],$vo[post_url]);?>" target="_blank">专家详情</a>
											<a onclick="openJesongChatByGroup(16524,20125);">预约咨询</a>	
										</div>
									</div>
