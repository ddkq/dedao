...
<script>
	...
	$('.header-nav-list li').on('mouseleave', function() {
		...
		<div class="main_content clearfix">
			<div class="case_box">
				<h3 class="term_title">爱牙案例</h3>
				<ul class="term_list clearfix">
					<?php $child = sp_get_child_terms(108); ?>
					<?php if(is_array($child)): $i = 0; $__LIST__ = $child;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; $on = $term_id==$vo[term_id]?'class="on"':'' ?>
						<li <?php echo ($on); ?>><a href="/list/<?php echo ($vo["term_id"]); ?>.html"><?php echo ($vo["name"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
				</ul>
				<div class="bd">
					<ul class="main_list">
...
<script>
	...
	<script>
		jQuery(".case").slide({mainCell:".bd",effect:"leftLoop",autoPlay:true});
	</script>
</html>