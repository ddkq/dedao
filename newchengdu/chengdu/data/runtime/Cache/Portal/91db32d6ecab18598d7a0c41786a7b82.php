	...
	$('.header-nav-list li').on('mouseleave', function() {
		...
		<div class="main_content clearfix">
			<div class="LEFT">
				...
				<ul class="main_list">
					<?php if(is_array($list['posts'])): $i = 0; $__LIST__ = $list['posts'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li style="position: relative;">
							<a class="media list_recommend" href="<?php echo get_article_url($vo[tid],$vo[link_type],$vo[post_url]);?>">
								<div class="media-left media-middle">
									<?php $smeta = json_decode($vo['smeta'],true); ?>
									<img src="<?php echo ($smeta["thumb"]); ?>">
								</div>
								<div class="media-body info">
									<h4 class="media-heading"><?php echo ($vo["post_title"]); ?></h4>
									<p><?php echo mb_substr($vo[post_excerpt],0,90,'utf-8');?>...</p>
								</div>
							</a>
							<?php $extend = json_decode($vo['post_extend'],true); ?>
							<?php if($extend['activity_time'] == 1): ?><div class="activily on">正在<br>进行<i></i></div>
							<?php else: ?>
								<div class="activily off">已经<br>结束<i></i></div><?php endif; ?>
